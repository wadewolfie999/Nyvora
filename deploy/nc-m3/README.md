# NC-M3 Split-Edge Deployment Package
...
Caddy is the sole public HTTP listener. VPS loopback 2222, asus Tracker 3000,
...
1. Resolve and verify external image digests and the live amd64 build digests.
2. Create the public NATS trust chain and scoped client credentials; encrypt
   distributable credentials with SOPS/age.
3. Render VPS Caddy/`frps` files and asus rootless Quadlets into a temporary
   output directory; validate syntax without installing them.
4. Compare every rendered listener against `config/nc-m3/ports.yml` and the
   fresh live socket baseline.
5. Apply only the next node-scoped envelope from the bootstrap runbook.
