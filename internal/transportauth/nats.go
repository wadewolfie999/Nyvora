...
func ValidateNATSURL(mode runtimeconfig.Mode, nodeID, rawURL string) error {
	...
	if parsed.Scheme != "nats" && parsed.Scheme != "ws" && parsed.Scheme != "wss" {
	...
	if mode == runtimeconfig.Live && nodeID != "asus-node" && parsed.Scheme != "wss" {
		return fmt.Errorf("live %s agent requires NATS over wss", nodeID)
	}
	return nil
}
